    ...
    public class CustomerService
    {
        ...
        public async Task<List<CustomerDTO>> GetAllCustomerByTime(int year, int month = 0)
        {
            try
            {
                if (month == 0)
                {
                    using (var context = new CinemaManagementEntities())
                    {
                        var customer = await context.Customers.Where(c => !c.IsDeleted && c.CreateAt.Year == year).Select(c => new CustomerDTO
                        {
                            Id = c.CustomerID,
                            Name = c.CustomerName,
                ...
                else
                {
                    using (var context = new CinemaManagementEntities())
                    {
                        var customer = await context.Customers.Where(c => !c.IsDeleted && c.CreateAt.Year == DateTime.Today.Year && c.CreateAt.Month == month)
        ...
        public async Task<List<CustomerDTO>> GetTop5CustomerEmail()
        {
            try
            {
                using (var context = new CinemaManagementEntities())
                {
                    var cusStatistic = await context.Bills.Where(b => b.BillTime.Year == DateTime.Now.Year && b.BillTime.Month == DateTime.Now.Month)
