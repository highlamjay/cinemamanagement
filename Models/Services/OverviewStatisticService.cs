        ...
        public async Task<int> GetBillQuantity(int year, int month = 0)
        {
            try
            {
                using (var context = new CinemaManagementEntities())
                {
                    if (month == 0)
                    {
                        return await context.Bills.Where(b => b.BillTime.Year == year).CountAsync();
                    }
                    else
                    {
                        return await context.Bills.Where(b => b.BillTime.Year == year && b.BillTime.Month == month).CountAsync();
                    }
                }
            }
            catch (Exception e)
        ...
        public async Task<(decimal, decimal)> GetFullRevenue(CinemaManagementEntities context, int year, int month = 0)
        {
            try
            {
                ...
                if (month != 0)
                {
                    ...
                    decimal ProductRevenue = await context.ProductBillInfoes.Where(pB => pB.Bill.BillTime.Year == year && pB.Bill.BillTime.Month == month)
                                                    .SumAsync(pB => (decimal?)(pB.PricePerItem * pB.Quantity)) ?? 0;
                    decimal TicketRevenue = await context.Tickets.Where(t => t.Bill.BillTime.Year == year && t.Bill.BillTime.Month == month)
                ...
                {
                    decimal ProductRevenue = await context.ProductBillInfoes.Where(pB => pB.Bill.BillTime.Year == year)
                                                    .SumAsync(pB => (decimal?)(pB.PricePerItem * pB.Quantity)) ?? 0;
                    decimal TicketRevenue = await context.Tickets.Where(t => t.Bill.BillTime.Year == year)
                                                    .SumAsync(t => (decimal?)t.Price) ?? 0;
                    return (ProductRevenue, TicketRevenue);
                }
            }
            ...
            try
            {
                if (month == 0)
                {
                    ...
                    decimal LastYearProdExpense = await context.ProductReceipts
                             .Where(pr => pr.TimeOfReceipt.Year == year)
                             .SumAsync(pr => (decimal?)pr.ImportPrice) ?? 0;

                    //Repair Cost
                    var LastYearRepairCost = await context.Troubles
                             .Where(tr => tr.TroubleFinishDate != null && tr.TroubleFinishDate.Value.Year == year)
                             .SumAsync(tr => (decimal?)tr.TroubleRepairCost) ?? 0;
                ...
                else
                {
                    ...
                    decimal LastMonthProdExpense = await context.ProductReceipts
                             .Where(pr => pr.TimeOfReceipt.Year == year && pr.TimeOfReceipt.Month == month)
                             .SumAsync(pr => (decimal?)pr.ImportPrice) ?? 0;
                    //Repair Cost
                    var LastMonthRepairCost = await context.Troubles
                             .Where(tr => tr.TroubleFinishDate != null && tr.TroubleFinishDate.Value.Year == year && tr.TroubleFinishDate.Value.Month == month)
                             .SumAsync(tr => (decimal?)tr.TroubleRepairCost) ?? 0;
            ...
            try
            {
                using (var context = new CinemaManagementEntities())
                {
                    var MonthlyProdExpense = await context.ProductReceipts
                     .Where(b => b.TimeOfReceipt.Year == year)
                     .GroupBy(b => b.TimeOfReceipt.Month)
                     .Select(gr => new
                     {
                         Month = gr.Key,
                    ...
                    var MonthlyRepairCost = await context.Troubles
                         .Where(t => t.TroubleFinishDate != null && t.TroubleFinishDate.Value.Year == year)
                         .GroupBy(t => t.TroubleFinishDate.Value.Month)
                         .Select(gr =>
                         new
                         {
                             Month = gr.Key,
                             Outcome = gr.Sum(t => (decimal?)t.TroubleRepairCost) ?? 0
                         }).ToListAsync();
        ...
        public async Task<(List<decimal> DailyExpense, decimal ProductExpense, decimal RepairCost, string RepairRateStr)> GetExpenseByMonth(int year, int month)
        {
            ...
            try
            {
                ...
                using (var context = new CinemaManagementEntities())
                {
                    //Product Receipt
                    var MonthlyProdExpense = await context.ProductReceipts
                         .Where(b => b.TimeOfReceipt.Year == year && b.TimeOfReceipt.Month == month)
                         .GroupBy(b => b.TimeOfReceipt.Day)
                         .Select(gr => new
                         {
                             Day = gr.Key,
                    ...
                    //Repair Cost
                    var MonthlyRepairCost = await context.Troubles
                        .Where(t => t.TroubleFinishDate != null && t.TroubleFinishDate.Value.Year == year && t.TroubleFinishDate.Value.Month == month)
                        .GroupBy(t => t.TroubleFinishDate.Value.Day)
                        .Select(gr =>
                        new
                        {
                            Day = gr.Key,
